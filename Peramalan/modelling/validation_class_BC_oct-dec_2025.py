...
OUTPUT_DIR = os.path.join(CURR_DIR, "validation_output_oct_dec_2025")
HORIZON = 30
LOOKBACK = 14
SARIMAX_ORDER = (1, 1, 1)
...
def train_predict_sarimax(train_df, future_df, target_col='Sales'):
    ...
    forecast = result.get_forecast(steps=len(future_df), exog=future_df[exog_cols])
...
def train_predict_lstm(train_df, future_df, lookback=LOOKBACK, target_col='Sales'):
    ...
    for d in [train_df, future_df]:
        d['dayofweek_sin'] = np.sin(2 * np.pi * d['Date'].dt.dayofweek / 7)
        d['dayofweek_cos'] = np.cos(2 * np.pi * d['Date'].dt.dayofweek / 7)
...
def validate_class_month(class_name, dept, class_df, year, month, month_name):
    ...
    for target in ['Sales', 'Sales Qty']:
        ...
        lstm_pred = train_predict_lstm(train_df, test_df, target_col=target)
        ...
        hybrid_lstm_pred = train_predict_lstm(
            train_df_hybrid, test_df, target_col='SARIMAX_Residual'
        )
        ...
        # Hanya simpan SARIMAX untuk output CSV (model terbaik)
        ...
        def _save_plot(ax, fig, suffix):
            ...
            fig.autofmt_xdate()
            plt.tight_layout()
            path = os.path.join(class_output_dir, suffix)
    ...
    daily_df['Dept'] = dept
    daily_df['Class'] = class_name
    return metrics_summary, daily_df
...
def validate_class(class_name, dept, file_path):
    ...
    for year, month, month_name in VALIDATION_MONTHS:
        res = validate_class_month(class_name, dept, class_df, year, month, month_name)
        if res and len(res[0]) > 0:
            metrics, daily_df = res
            all_metrics.extend(metrics)
            all_daily_dfs.append(daily_df)

    if not all_daily_dfs:
        return [], pd.DataFrame()
    return all_metrics, pd.concat(all_daily_dfs, ignore_index=True)
...
def main():
    ...
    for dept in DEPARTMENTS:
        ...
        for cls in classes:
            ...
            if os.path.exists(file_path):
                res = validate_class(cls, dept, file_path)
                if res and len(res[0]) > 0:
                    metrics, daily_df = res
                    all_metrics.extend(metrics)
                    all_daily_dfs.append(daily_df)
    ...
    if all_daily_dfs:
        df_daily = pd.concat(all_daily_dfs, ignore_index=True)
        cols = ['Date', 'Dept', 'Class'] + [c for c in df_daily.columns if c not in ['Date', 'Dept', 'Class']]
        df_daily = df_daily[cols]
        for cls in classes:
            cls_daily = df_daily[df_daily['Class'] == cls]
            if not cls_daily.empty:
                class_output_dir = os.path.join(OUTPUT_DIR, f"class_{cls}")
                if not os.path.exists(class_output_dir):
                    os.makedirs(class_output_dir)
                daily_csv_path = os.path.join(class_output_dir, f"daily_forecast_class_{cls}.csv")
                cls_daily.to_csv(daily_csv_path, index=False)
                print(f"Data harian prediksi (Daily Forecast) class {cls} disimpan ke: {daily_csv_path}")
        ...
        for cls in classes:
            ...
            if not cls_metrics.empty:
                ...
                metrics_csv_path = os.path.join(
                    class_output_dir, f"validation_metrics_summary_class_{cls}_oct_dec_2025.csv"
                )
                cls_metrics.to_csv(metrics_csv_path, index=False)
                print(f"\nMetrics summary class {cls} disimpan ke: {metrics_csv_path}")
