...
OUTPUT_DIR = os.path.join(CURR_DIR, "validation_output", "class_BC_LSTM")
HORIZON = 30
LOOKBACK = 14
SARIMAX_ORDER = (1, 1, 1)
...
def validate_class(class_name, dept, file_path):
    ...
    for target in ['Sales', 'Sales Qty']:
        ...
        def _save_plot(ax, fig, filename):
            ...
            fig.autofmt_xdate()
            plt.tight_layout()
            path = os.path.join(class_output_dir, filename)
        ...
        ax.set_title(f"Validasi ({target}) — Dept: {dept.upper()} | Class: {class_name}", fontsize=13)
        _save_plot(ax, fig, f"val_plot_{dept}_class_{class_name}_{fname_suffix}.png")

        # 2. Actual vs SARIMAX
        fig, ax = plt.subplots(figsize=(12, 6))
        ax.plot(test_dates, y_actual,     label=f'Actual {target}', marker='o', color='black',    linewidth=2)
        ax.plot(test_dates, sarimax_pred, label='SARIMAX',           linestyle='--', color='steelblue', linewidth=2)
        ax.set_title(f"Actual vs SARIMAX ({target}) — Dept: {dept.upper()} | Class: {class_name}", fontsize=13)
        _save_plot(ax, fig, f"val_plot_{dept}_class_{class_name}_{fname_suffix}_SARIMAX.png")

        # 3. Actual vs LSTM
        fig, ax = plt.subplots(figsize=(12, 6))
        ax.plot(test_dates, y_actual,  label=f'Actual {target}', marker='o', color='black',      linewidth=2)
        ax.plot(test_dates, lstm_pred, label='LSTM',              linestyle='--', color='darkorange', linewidth=2)
        ax.set_title(f"Actual vs LSTM ({target}) — Dept: {dept.upper()} | Class: {class_name}", fontsize=13)
        _save_plot(ax, fig, f"val_plot_{dept}_class_{class_name}_{fname_suffix}_LSTM.png")

        # 4. Actual vs Hybrid
        fig, ax = plt.subplots(figsize=(12, 6))
        ax.plot(test_dates, y_actual,    label=f'Actual {target}',    marker='o', color='black', linewidth=2)
        ax.plot(test_dates, hybrid_pred, label='Hybrid SARIMAX-LSTM', linestyle='-.', color='green', linewidth=2)
        ax.set_title(f"Actual vs Hybrid SARIMAX-LSTM ({target}) — Dept: {dept.upper()} | Class: {class_name}", fontsize=13)
        _save_plot(ax, fig, f"val_plot_{dept}_class_{class_name}_{fname_suffix}_Hybrid.png")
        ...
        for cls in classes:
            ...
            if not cls_metrics.empty:
                ...
                metrics_csv_path = os.path.join(
                    class_output_dir, f"validation_metrics_summary_class_{cls}.csv"
                )
                cls_metrics.to_csv(metrics_csv_path, index=False)
                print(f"\nMetrics Class {cls} disimpan ke: {metrics_csv_path}")
