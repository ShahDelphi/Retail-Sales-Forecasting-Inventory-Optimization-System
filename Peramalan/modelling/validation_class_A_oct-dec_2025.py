...
def main():
    ...
    if all_daily_dfs:
        ...
        daily_csv_path = os.path.join(OUTPUT_DIR, "daily_forecast_class_A.csv")
        df_daily.to_csv(daily_csv_path, index=False)
        print(f"\nData harian prediksi (Daily Forecast) disimpan ke: {daily_csv_path}")

    if all_metrics:
        df_metrics = pd.DataFrame(all_metrics)
        metrics_csv_path = os.path.join(OUTPUT_DIR, "opt_validation_metrics_summary.csv")
        df_metrics.to_csv(metrics_csv_path, index=False)
        print(f"Metrics summary disimpan ke: {metrics_csv_path}")
