...
print(f"→ Baris kategori weekend: {df_ts['Weekend'].sum():,}")
...
print(f"→ Baris dengan promo aktif: {df_ts['Promo'].sum():,}")
...
for dept in ["FRUIT", "VEGETABLE"]:
    ...
    if unmatched > 0:
        print(f"  ⚠️  {unmatched:,} baris tidak match (di-skip)")
    ...
    for kelas in ["A", "B", "C"]:
        ...
        print(f"  ✅ Kelas {kelas}: {len(df_kelas):,} baris → timeseries_{kelas}.csv")
